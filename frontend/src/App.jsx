...
function App() {
  ...
  // --- Gamepad / PS5 Controller Integration ---
  useEffect(() => {
    let frameId;
    let prevButtons = Array(20).fill(false);
    
    const handleGamepadConnected = (e) => {
      triggerFlash(`🎮 CONTROLLER LINKED: ${e.gamepad.id.substring(0, 20)}...`);
      playSciFiSound('engage');
      addLog(`Gamepad Connected: ${e.gamepad.id}`, 'success');
    };
    ...
    const pollGamepad = () => {
      if (role === 'surgeon') {
        ...
        if (gp) {
          // Left Joystick for Robot Arm Movement (axes 0 and 1)
          ...
          if (Math.abs(axisX) > 0.1 || Math.abs(axisY) > 0.1) {
            setLocalMouse(prev => {
              let newX = prev.x + (axisX * 0.8);
              let newY = prev.y + (axisY * 0.8);
              newX = Math.max(0, Math.min(100, newX));
              newY = Math.max(0, Math.min(100, newY));

              const distToTumor = Math.sqrt(Math.pow(newX - 20, 2) + Math.pow(newY - 25, 2));
              const inDanger = distToTumor < 12;
              setIsVibrating(inDanger);

              // Force Feedback (Rumble) Support for PS4/PS5 controllers
              if (inDanger && gp.vibrationActuator && gp.vibrationActuator.playEffect) {
                gp.vibrationActuator.playEffect("dual-rumble", {
                  startDelay: 0, duration: 100, weakMagnitude: 0.8, strongMagnitude: 1.0
                }).catch(() => {});
              }

              // Socket Sync Throttle
              const now = performance.now();
              if (now - netThrottleRef.current > 16) {
                if (socket) socket.emit('cursor-move', { 
                  x: newX, y: newY, danger: inDanger, 
                  organ: selectedOrgan, systemState, vision: aiVisionOverlay,
                  cutting: isCutting, point: incisionPoint
                });
          ...
          // Trigger R2 (Button 7) or X (Button 0) for Incision
          const isActionPressed = gp.buttons[7]?.pressed || gp.buttons[0]?.pressed;
          if (isActionPressed && !prevButtons[0]) {
             setIsCutting(true);
             playSciFiSound('scan'); // Using scan as incision noise
          } else if (!isActionPressed && prevButtons[0]) {
             setIsCutting(false);
          }
          prevButtons[0] = isActionPressed;
          
          // Square Button (Button 2) -> Run AI Scan / Workflow
          const isSquare = gp.buttons[2]?.pressed;
          if (isSquare && !prevButtons[2]) {
             if (systemState === 'IDLE' && selectedOrgan) runSimulationWorkflow();
          }
          prevButtons[2] = isSquare;

          // Triangle Button (Button 3) -> Toggle Haptics
          const isTriangle = gp.buttons[3]?.pressed;
          if (isTriangle && !prevButtons[3]) {
             setHapticsEngaged(prev => !prev);
             playSciFiSound('engage');
          }
          prevButtons[3] = isTriangle;
        }
      }
  ...
  }, [role, selectedOrgan, systemState, aiVisionOverlay, isCutting, incisionPoint, socket]);
